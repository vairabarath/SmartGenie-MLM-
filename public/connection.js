...
async function handleNetworkChange() {
  try {
    await browserWallet.request({
      ...
      params: [{ chainId: "0x15EB" }], // 0x204 for opBNB Mainnet // 0x15EB for opBNB Testnet (5611 in hex)
    });
...
async function test() {
  try {
    ...
    const userList = await contract.methods.users(currentAccount).call();
    console.log(userList);
  } catch (error) {
    console.error("Fetching User List Data:", error);
    return 0;
  }
}
    ...
    for (i = 1; i <= 9; i++) {
      if (i <= Number(userData.levelEligibility)) {
        ...
        const incomeCount = await contract.methods
          .getUserIncomeCount(currentAccount, i)
          .call();
